...
const Input: FC<IProps> = ({ label, isInvalid, messageError, ...props }) => {
    const classError =
        isInvalid && !props.readOnly ? "border-red-500 text-red-500 placeholder:text-red-500" : "border-zinc-400";
    ...
    return (
        <>
            ...
            />
            {isInvalid && !props.readOnly && <span className="text-red-500 text-xs ml-2">{messageError}</span>}
        </>
    );
};
